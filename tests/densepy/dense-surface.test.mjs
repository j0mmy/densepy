...
import { compileToPython, runSource, densify } from '../../src/densepy/compiler.mjs';
...
τ('densify minimizes tokens while preserving semantics and staying idempotent', () => {
  ...
  const b = runSource(dense);
  assert.equal(b.status, 0, b.stderr + '\n' + dense);
  assert.equal(a.stdout, b.stdout);
});

τ('densify protects strings, comments, and bracket-continuation lines', () => {
